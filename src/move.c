...
static unsigned char neigh_bitset(struct hive *hive,
	struct vec3 *pos, enum hive_side side)
{
...
void hive_moves_for_ant(struct hive *hive, struct vec3 *pos)
{
    ...
    while (arrlen(posQueue)) {
        ...
        unsigned char neigh = neigh_bitset(hive, &currentPos, 
        	                       HIVE_WHITE & HIVE_BLACK);
...
{
	unsigned char neigh = neigh_bitset(hive, pos, HIVE_WHITE & HIVE_BLACK);
    unsigned char slide = slide_bitset(neigh);
    struct node {
    	struct vec3 pos;
    ...
    struct vec3 *movePos = NULL;
    arrput(nodeQueue, ((struct node){*pos, (struct vec3){0, 0, 0}, 0}));

    while (arrlen(nodeQueue)) {
    	struct node node = arrpop(nodeQueue);
    	if (node.depth == 2) {
    		arrput(movePos, node.pos);
    	}
    	unsigned char neigh = neigh_bitset(hive, 
    		&node.pos, HIVE_WHITE & HIVE_BLACK);
    	unsigned char slide = slide_bitset(neigh);
    	for (int i = 0; i < 6; i++) {
            if(slide & HIVE_DIRECTION_BIT(i)) {
                struct vec3 adjPos = vec_move(&node.pos, i);
                if (memcmp(&adjPos, &node.previousPos, sizeof node.previousPos) != 0) {
                	arrput(nodeQueue, 
                		((struct node){adjPos, node.pos, node.depth + 1}));
                }
            }
    	}
    }
...
void hive_moves_for_grasshopper(struct hive *hive, struct vec3 *pos)
{
	...
	for (int i = 0; i < 6; i++) {
		...
		piece_t adjPiece = hive->grid[adjPos.x + adjPos.y * GRID_COLUMNS];
		if (adjPiece) {
			while (adjPiece) {
